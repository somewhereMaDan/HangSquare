...
export const Get_all_user_info = async (req, res) => {
  try {
    const all_user_info = await UserModel.find({});
    return res
      .status(200)
      .json({ message: "Fetched all User Info", all_user_info });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Get_all_user_info 500 Error" });
  }
};
...
export const GetUserFriends = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ FriendsArr: FriendsArr, FriendsId: FriendsId });
  } catch (err) {
...
export const addRemoveFriend = async (req, res) => {
  try {
    ...
    if (user.Friends.includes(friendId)) {
      ...
      const removeUserFromFriend = FriendUser.Friends.filter(
        (id) => id !== UserId
      );
    ...
    return res
      .status(200)
      .json({ message: "Friend List Updated", SingleUser: user });
  } catch (err) {
...
export const CompleteProfile = async (req, res) => {
  try {
    ...
    const { Firstname, Lastname, Location, Occupation, Instagram, Linkdin } =
      req.body;
    ...
    return res.status(200).json({
      ...
      Occupation: user.Occupation,
      Location: user.Location,
      Linkdin: user.SocialProfile.get("Linkdin"),
      Instagram: user.SocialProfile.get("Instagram"),
      user: user,
    });
  } catch (err) {
